  ...
  return (
    <nav className="sticky top-0 z-50 bg-opacity-10 backdrop-blur-md">
      <div className="navbar flex items-center justify-between p-2 px-4 md:px-10">
        ...
        {/* Desktop Tags */}
        <div className="hidden xl:flex gap-6 text-black questrial-regular text-lg tracking-normal">
          {tags.map((tag, index) => (
            <motion.a
              key={index}
              href={tag.href}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className=" xl:hidden bg-opacity-90 backdrop-blur-md px-6 pt-4 pb-6 flex flex-col gap-4 text-lg questrial-regular shadow-lg"
          >
            {tags.map((tag, index) => (
              <motion.a
                key={index}
